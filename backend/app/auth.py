...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(db.get_session)):
    ...
    try:
        ...
        raise credentials_exception
    user = db.query(db.dbm.User).filter(db.dbm.User.username == token_data.username).first()
    if user is None:
        raise credentials_exception
    return user
